...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (whatsappBtn) {
    const contact = (window.IftikarCMS && window.IftikarCMS.data && window.IftikarCMS.data.contact) || {};
    const whatsappDigits = (contact.whatsapp || '').replace(/[^0-9]/g, '');

    if (whatsappDigits) {
      const greeting = 'Hi Iftikar, I\'d like to talk about a project.';
      whatsappBtn.href = 'https://wa.me/' + whatsappDigits + '?text=' + encodeURIComponent(greeting);
    } else if (contact.email) {
      whatsappBtn.href = 'mailto:' + contact.email;
      whatsappBtn.removeAttribute('target');
      whatsappBtn.textContent = '';
      const label = document.createElement('span');
      label.textContent = 'Email Me ';
      whatsappBtn.appendChild(label);
      const arrow = document.createElement('span');
      arrow.className = 'arrow';
      arrow.textContent = '↗';
      whatsappBtn.appendChild(arrow);
    } else {
      whatsappBtn.href = '#';
    }
  }
